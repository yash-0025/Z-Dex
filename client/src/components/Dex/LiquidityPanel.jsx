...
export const LiquidityPanel = () => {
    ...
    const poolShare = useMemo(() => {
        ...
        if (isNaN(parsedLiquidityAmount) || isNaN(parsedTokenReserve) || parsedLiquidityAmount <= 0) {
            return '0.00';
        }

        // Calculate pool share as: new liquidity / (existing reserve + new liquidity) * 100
        const totalAfterAddition = parsedTokenReserve + parsedLiquidityAmount;
        const sharePercentage = (parsedLiquidityAmount / totalAfterAddition) * 100;
        
        // Cap at 100% and ensure it's a reasonable number
        return Math.min(sharePercentage, 100).toFixed(2);
